...
class SupportPhaseSeeder extends Seeder
{
    /**
    ...
    public function run()
    {
        ...
        $path = storage_path('app/public/support_phases.xlsx');
            ...
            if ($xlsx = SimpleXLSX::parse($path)) {
                foreach ($xlsx->rows() as $row) {
                    if ($i > 0) {
                        ...
                        DB::table('support_phases')->insert($data);
                    }
                    $i++;
                }
                DB::statement("SELECT setval('support_phases_id_seq', (SELECT MAX(id) FROM support_phases));");
            } else {
