...
//get view user data and set it as default value for each input 
const userParam: any = router.currentRoute.value.params
const { data: userData } = await useApi<any>(createUrl('/v1/users/'+ userParam?.id))
const form = ref({
  name: userData.value?.name || '',
  email: userData.value?.email || '',
  status: userData.value?.status || false,
  school_id: userData.value?.school_id || '',
  roles: userData.value?.roles.map((role: any) => role.id) || [],
...
const schools = ref([userData.value?.school])

async function handleSubmit(){
  const res = await $api('/v1/users/' + userData.value?.id, {
    method: 'PATCH',
    body: { ...form.value,roles: JSON.stringify(form.value.roles)  }
  })
  ...
  timeout = setTimeout(async () => {
    ...
    try {
      const { data: schoolData } = await useApi<any>(
        createUrl(`/v1/schools?page=1&limit=10&search=${query}`)
      )
...
watch(school_search, query => {
  query && query !== form.value.school_id && querySelections(query)
})
      ...
      <VCol>
        <VForm @submit.prevent.left="() => { handleSubmit() }">
          <VRow>
            ...
            <!-- 👉 តួនាទី -->
            <VCol cols="12" md="6" class="flex gap-2 items-end">
              <AppSelect v-model="form.roles"
                :items="roles"
                item-title="name"
                item-value="id"
            ...
            <!-- 👉 ដាក់អោយដំណើរការ -->
            <VCol cols="12" class="flex gap-2 items-center">
              <VSwitch
                v-model="form.status"
              />
    ...
    <VDialog v-model="isDialogVisible" max-width="600">
      ...
      <VCard>
        ...
        <VCardText class="d-flex justify-end flex-wrap gap-3">
          ...
          <VBtn color="primary" @click="handle_createrole()">
            រក្សាទុក
          </VBtn>
        </VCardText>
      </VCard>
